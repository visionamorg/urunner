...
- [ ] Event detail page (`/events/:id`) shows a map with the start location marker
- [ ] Map uses OpenStreetMap via Leaflet.js (free, no API key for basic use)
- [ ] Clicking the marker opens directions in Google Maps (`https://maps.google.com?q=lat,lng`)
- [ ] If no GPS data: shows a static placeholder with the location name text

### Event Creation — Location Fields
- [ ] Event creation form (admin) adds:
...
- [ ] `Event` model: add `locationName`, `latitude`, `longitude`, `routeGpxUrl` columns

### Route Display
- [ ] If a GPX file was uploaded, draw the route as a polyline on the map
- [ ] Route stats shown beside map: Total distance, Elevation gain, Estimated time at avg pace
- [ ] GPX file stored in `/uploads/routes/` (same upload volume as US-010)

### Events List — Location Badge
- [ ] Event card on the list shows location name as a small badge with a pin icon
- [ ] "Online" badge if no GPS location set

### Upcoming Events Widget on Dashboard
- [ ] Dashboard upcoming events widget shows location name under each event title
