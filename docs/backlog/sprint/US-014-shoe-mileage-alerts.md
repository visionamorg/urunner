...
- [ ] Gear tab on profile shows each shoe with a progress bar: current KM / max KM
- [ ] Color: green (< 70%), yellow (70–90%), red (> 90%)
- [ ] Progress bar shows percentage and "X km remaining"

### Alert Thresholds
- [ ] At 80%: orange warning badge on the shoe card — "Getting worn — consider replacing soon"
- [ ] At 100%: red "Retire" badge — "This shoe has exceeded its recommended distance"
- [ ] Retired shoes can be marked as "Retired" — they disappear from the active gear list but stay in history

### Notification
- [ ] When a new activity is synced and it pushes a shoe over the 80% threshold, a notification is sent:
  - "Your Nike Vaporfly (442km) has reached 80% of its recommended life. Consider replacing soon."
- [ ] Notification also appears as a real-time toast (if US-012 is implemented)

### Default Shoe Auto-Assignment
- [ ] When a shoe is set as "default", all new activities auto-assign to that shoe unless overridden
- [ ] Activity detail page shows which shoe was used (if assigned), with a "Change shoe" button
- [ ] `PUT /api/shoes/{shoeId}/assign-activity/{activityId}` endpoint

### Retire Flow
- [ ] Shoe card in profile has "Retire" button (visible when > 90% or manually)
- [ ] Retired shoes are hidden from activity assignment but shown in "Archived Gear" section
