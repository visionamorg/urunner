...
### Acceptance Criteria
- [x] Add a `price` (BigDecimal) field to the `Programme` model. If `price == 0` or null, it's free.
- [x] If `price > 0`, the Programme Details page shows "Unlock Programme for $X.XX" instead of "Enroll".
- [x] Clicking the button routes to Stripe Checkout using the `PROGRAMME` entity type in metadata.
- [x] The backend must block `GET /api/programmes/{id}/workouts` for users who are not explicitly enrolled.
- [x] When the Stripe webhook fires and publishes `PaymentSuccessfulEvent(PROGRAMME, id)`, the user's `ProgrammeEnrollment` status flips from `PENDING` to `ACTIVE`.
- [x] Upon returning to the app, the 12 weeks of workouts instantly populate on their personal calendar.
