...
### Acceptance Criteria
- [x] Add `routeGpxUrl` and `elevationGainMeters` to the `Event` entity in the backend.
- [x] Admins can upload a `.gpx` file when creating or editing an event (stored securely in an S3 bucket or local storage).
- [x] Display an interactive map component on the Event details page rendering the GPX trace line.
- [x] Display an elevation profile chart (e.g., using ApexCharts) extracted from the GPX data beneath the map.
- [x] Provide members a "Download GPX" button so they can push the route to their Garmin or Coros watches.
